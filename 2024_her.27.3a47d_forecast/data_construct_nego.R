...
# windows()
# 
# F01 <- quantMeans(window(NSH,start=2010)@harvest[ac(0:1),])
# plot(F01)+ylim(0,0.07)+geom_hline(yintercept = 0.05,col='red')
# 
# 
C01 <- window(NSH,start=2010)@catch.n*window(NSH,start=2010)@catch.wt
C01 <- C01[ac(0:1),]
quantSums(C01)
...
stf.options <- c("Headline advice corrected",
                 'C-TAC rule_B rollover_0% transfer',
                 'C-TAC rule_B rollover_100% transfer',
                 'C-TAC rule_B rollover_NSAS share transfer',
                 'C-TAC rule_B decrease_0% transfer',
                 'C-TAC rule_B decrease_100% transfer',
                 'C-TAC rule_B prop_0% transfer',
                 'C-TAC rule_B prop_100% transfer',
                 'C-TAC rule_F01=0.05_0% transfer',
                 'C-TAC rule_F01=0.05_100% transfer')
...
transferTab[DtY,]$D_TAC.NS  <- transferTab[DtY,]$CD_TAC_EU*catchAreaTab[DtY,]$DshareEU3a
catchAreaTab$BDuptake[catchAreaTab$year == an(DtY)]   <- catchAreaTab$B[catchAreaTab$year == an(DtY)]/(TACTab[DtY,]$B+TACTab[DtY,]$D-transferTab[DtY,]$CD_TAC_EU*catchAreaTab[DtY,]$DshareEU3a)
#catchAreaTab$BDuptake[catchAreaTab$year == an(DtY)]   <- catchAreaTab$B[catchAreaTab$year == an(DtY)]/(TACTab[DtY,]$B+TACTab[DtY,]$D-transferTab[ImY,]$CD_TAC_EU*catchAreaTab[DtY,]$DshareEU3a)
...
TAC_var$DtransferFcY <- TAC_var$DtransferImY/TACTab[ImY,]$D
...
stf.table <- array(NA,dim=c(c(length(stf.options)+1),24,3),
                   dimnames=list("options"=c("intermediate year",stf.options),
                                 "values" =c("Fbar 2-6 A",
                                             ...
                                             "Fbar 2-6",
                                             "Fbar 0-1",
                                             "topSlice scenario C-D slice",
                                             "topSlice scenario A-B",
                                             "topSlice scenario C-D",
                                             "Catch A",
                                             "Catch B",
                                             "Catch C",
                                             "Catch D",
...
stf.table[1,"SSB_FcY",]     <- quantSums(stf@stock.n[,ImY,1] * stf@stock.wt[,ImY,1] *
                                                                           exp(-unitSums(stf@harvest[,ImY])*stf@harvest.spwn[,ImY,1]-stf@m[,ImY,1]*stf@m.spwn[,ImY,1]) *
                                                                           stf@mat[,ImY,1])
stf.table[1,"SSB_CtY",]     <- quantSums(stf@stock.n[,ImY,1] * stf@stock.wt[,ImY,1] *
                                           exp(-unitSums(stf@harvest[,ImY])*stf@harvest.spwn[,ImY,1]-stf@m[,ImY,1]*stf@m.spwn[,ImY,1]) *
                                           stf@mat[,ImY,1])
...
save(CATCH, 
     ...
     TAC_var, 
     TACS,
     file=file.path('.','data',paste0('input_',assessment_year,'_nego.RData')))
