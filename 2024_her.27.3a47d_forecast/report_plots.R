...
taf.png("TAC_advice_A")
print(ggplot(subset(adviseTAC,year>=2018 & fleets == 'A'),aes(x=year,y=data,col=type))+
        geom_line()+
        ylab('A fleet - advice and TAC')+
        facet_wrap(~fleets,scales='free'))
dev.off()
...
ggplot()+
        ...
        geom_text(data=subset(df.all,yearType!='CtY' & assess_year == assessment_year),aes(x=ssb*1e-6,y=fbar,label=year),hjust=0, vjust=0,col='purple')+
        geom_point(data=subset(df.all,yearType!='CtY' & assess_year == assessment_year),aes(x=ssb*1e-6,y=fbar),col='purple')+
        xlab('SSB (millions tonnes)')+
        geom_vline(xintercept = 828874*1e-6,col='red')+
        geom_vline(xintercept = 1049521*1e-6,col='yellow')+
        geom_vline(xintercept = 1130747*1e-6,col='blue')
dev.off()
